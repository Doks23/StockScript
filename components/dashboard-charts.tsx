...
export function DashboardCharts({ trades, metrics, userCapital, groupedTrades }: {
  ...
  const [timeRange, setTimeRange] = useState<"10" | "1w" | "1m" | "3m" | "all">("10");
  ...
  const { equityCurve, recentStats, monthlyTotals } = useMemo(() => {
    ...
    const equityCurve = sorted.map((t: any) => {
      const pnl = t.status === "CLOSED" ? t.netPnl : (t.closingPrice ? calculateOpenPnl(t) : 0);
      cumulative += pnl;
      return {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-3xl border border-[#f0e7cf] bg-white p-5 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <p className="text-sm uppercase tracking-wider text-slate-500">Recent Performance</p>
          ...
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-2 text-profit">
              ...
              <span className="text-3xl font-display font-semibold">{recentStats.winners}</span>
            </div>
            <span className="text-xl text-slate-400">vs</span>
            <div className="flex items-center gap-2 text-loss">
              <ArrowDown className="w-8 h-8 relative top-1" />
              <span className="text-3xl font-display font-semibold relative top-1">{recentStats.losers}</span>
            </div>
          </div>
          <div className="mt-4 pt-4 border-t border-slate-100 flex justify-between text-sm text-slate-500">
            <span className="text-slate-400">Sample Size</span>
            <span className="font-medium text-ink">{recentStats.total} trades</span>
          </div>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="rounded-3xl border border-[#f0e7cf] bg-white p-5 shadow-sm">
          <p className="text-sm font-medium text-slate-700 mb-4">Portfolio Allocation % (Open)</p>
          <div className="h-48 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={trades.filter((t: any) => t.status === "OPEN").slice(0, 6).map((t: any) => ({
                symbol: t.symbol,
                allocationPct: userCapital > 0 ? ((t.capitalUsed || (t.weightedEntryPrice * t.totalEntryQty) || 0) / userCapital) * 100 : 0,
                ...
                <Bar dataKey="allocationPct" fill="var(--accent)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="h-48 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyTotals} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                ...
                <Bar dataKey="pnlPct" fill="var(--accent)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
